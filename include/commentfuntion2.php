...
// function to fetch posts(all) from the database and returning them to the HTML
	...
	function fetch_comment($slug)	{
		...
		$sql_comment = "SELECT commments.comment, users.username FROM commments , users LEFT JOIN username ON c.uid = u.id WHERE commments.post_slug = '$post_slug';";
		echo $sql_comment;
		$result_comment = mysqli_query($connection_pointer, $sql_comment);
		echo mysqli_error($result_comment);
